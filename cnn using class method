...
print('loading data')
data = 'data/data'  # Path to the dataset
labels = []
images = []
image_size = 240
...
batch_img = np.array(batch_img).astype(np.float64)
            ...
            for mp in range(no_filters):
                for h in range(nh):               
                    for w in range(nw): 
                        ...
                        horiz_start = w * stride
                        horiz_end = horiz_start + f
                        self.dA_prev[: ,vert_start:vert_end , horiz_start:horiz_end] += (weight[mp ,: ,:] * dZ[mp ,h ,w])#.astype(np.float32)
            print('da prev',self.dA_prev.shape)
                      #db[c,:,:]+=dZ[c,h,w]
...
class dense:
    ...
    def back_propagation_dense(self, x, y):
        ...
        dZ4 = ls  
        #print('dZ3',dZ3)
        dA3 = np.dot(dZ4, theta3.T) * relu_1.relu_derivatived(self.a3)
        dZ3 = dA3
    
        dA2 = np.dot(dZ3, theta2.T) * relu_1.relu_derivatived(self.a2)
        dZ2 = dA2
        #print('dZ2',dZ2.shape)
    
        dA1 = np.dot(dZ2, theta1.T) * relu_1.relu_derivatived(self.a1)
        #print('dA1',dA1.shape)
...
def conv_forward(batch_im):
    ...
    result_4 = layer_1.conv2(l4_filter ,result_relu3 ,b4 ,4)
    result_relu4 = relu_1.relu(result_4)
  
    
    result_5 = layer_1.conv2(l5_filter ,result_relu4 ,b5 ,5)
    result_relu5 = relu_1.relu(result_5)
    pool.update_parms(result_relu5 , s , ps)
    pooling_5 , cache5  = pool.pooling_max()
    print('last pooling layer result',pooling_5.shape)
...
def trainf( lr, epochs):
    ...
    #######################################
    l1_filter = np.abs( np.random.randn(96 ,11 ,11))
    b1 = np.zeros((96 ,1 , 1))
    
    l2_filter = np.abs( np.random.randn(256 ,5 ,5))
    b2 = np.zeros((256 ,1 , 1))
    
    l3_filter = np.abs( np.random.randn(384 ,3 ,3))
    b3 = np.zeros((384 ,1 , 1))

    l4_filter = np.abs( np.random.randn(384 ,3 ,3))
    b4 = np.zeros((384 ,1 , 1))
    
    l5_filter = np.abs( np.random.randn(256 ,3 ,3))
    b5 = np.zeros((256 ,1 , 1))
    ...
    theta1 = np.abs( np.random.rand(no_l2, no_l3))
    bias1 = np.zeros((1, no_l3))
    theta2 = np.abs( np.random.rand(no_l3, no_l4) )
    bias2 = np.zeros((1, no_l4))
    theta3 = np.abs( np.random.rand(no_l4, no_out) )
